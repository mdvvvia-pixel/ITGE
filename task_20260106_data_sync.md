...
- `TableGraphEditor/src/helpers/METHODS_FOR_MLAPP.m` (callback `tblDataCellEdit`)
...
**Success Criteria:**
- [x] При редактировании таблицы график обновляется
- [x] При перетаскивании точки таблица обновляется (уже реализовано в drag_and_drop)
- [x] Валидация числовых данных работает
- [x] Метки не редактируются (read-only)
- [x] Используется `drawnow limitrate` для оптимизации
...
**Deliverables:**
- [x] Метки не редактируются
- [x] Остальные ячейки редактируемые
...
- ✅ Создана функция `validateNumericData.m` в папке `helpers/`:
  - Валидирует числовые значения (числа, строки, которые можно преобразовать в числа)
  - Проверяет, что значение не пустое и является скаляром
  - Поддерживает преобразование строк в числа
  - Разрешает NaN и Inf (можно изменить, раскомментировав проверку)
  - Обрабатывает edge cases (пустые значения, массивы, нечисловые строки)
- ✅ Добавлен callback `tblDataCellEdit` в `METHODS_FOR_MLAPP.m`:
  - Обработчик редактирования ячеек таблицы (CellEditCallback)
  - Проверяет, не является ли ячейка меткой (read-only для первой строки/столбца)
  - Валидирует числовые значения через validateNumericData
  - Обновляет currentData при успешной валидации
  - Обновляет график через updateGraph после изменения данных
  - Использует drawnow limitrate для оптимизации производительности
  - Восстанавливает предыдущее значение при ошибках валидации
  - Безопасная работа со свойствами app (с fallback на UserData)
  - Поддержка обоих режимов построения графика (columns/rows)
- ✅ Реализована защита меток от редактирования:
  - В режиме "по столбцам": первая строка (row == 1) защищена от редактирования
  - В режиме "по строкам": первый столбец (col == 1) защищен от редактирования
  - При попытке редактирования метки показывается предупреждение и значение восстанавливается
...
- [x] Функция `validateNumericData` реализована
- [x] Callback `tblDataCellEdit` реализован
- [x] Метки не редактируются
- [x] Валидация данных работает
- [x] График обновляется при редактировании таблицы
- [x] Оптимизация обновлений реализована
...
1. **`validateNumericData.m`** (создан)
   - Валидирует числовые значения (числа, строки, которые можно преобразовать в числа)
   - Проверяет, что значение не пустое и является скаляром
   - Поддерживает преобразование строк в числа
   - Разрешает NaN и Inf (можно изменить, раскомментировав проверку)
...
2. **`tblDataCellEdit`** (добавлен в METHODS_FOR_MLAPP.m)
   - Callback для обработки редактирования ячеек таблицы (CellEditCallback)
   - Проверяет, не является ли ячейка меткой (read-only для первой строки/столбца)
   - Валидирует числовые значения через validateNumericData
   - Обновляет currentData при успешной валидации
   - Обновляет график через updateGraph после изменения данных
...
- Функция валидации числовых данных работает
- Callback для редактирования таблицы реализован
- Защита меток от редактирования работает
- Обновление графика при редактировании таблицы реализовано
- Оптимизация обновлений через drawnow limitrate реализована
...
- Функция валидации числовых данных работает
- Callback для редактирования таблицы реализован и протестирован
- Защита меток от редактирования работает
- Обновление графика при редактировании таблицы работает
- Оптимизация обновлений через drawnow limitrate реализована
- Обработка ошибок реализована во всех функциях
- Все функции соответствуют MATLAB_STYLE_GUIDE
- ✅ **Протестировано пользователем - работает корректно!**

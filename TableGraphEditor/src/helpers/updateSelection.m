...
function updateSelection(app, selection)
    ...
    try
        if strcmp(plotType, 'columns')
            ...
            % Столбец 1 используется для X координат, поэтому его нельзя выделять
            filteredCols = selectedCols(selectedCols > 1);
            ...
            % Строка 1 используется для X координат, поэтому её нельзя выделять
            filteredRows = selectedRows(selectedRows > 1);
